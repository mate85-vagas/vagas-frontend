...
function ManageEmailList() {
  ...
  const { emailLists, refresh: refreshEmailLists } = useGetEmailLists()
  ...
  const saveEmailLists = () => {
    ...
    if (newEmails.length > 0)
      createEmailList(newEmails, emailListState).then(async () => {
        if (deleteEmails.length > 0) await deleteEmailList(deleteEmails)
        refreshEmailLists()
      })
    else if (deleteEmails.length > 0)
      deleteEmailList(deleteEmails).then(() => refreshEmailLists())
  }
